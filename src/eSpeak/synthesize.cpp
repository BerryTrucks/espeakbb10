...
//         5: interrupt and flush current text.
	...
	if (0 == Generate(phoneme_list,&n_phoneme_list,0)) {
		// Yuri Salnikov 11/17/2012
		// Change the voice (is necessary) only if finished the phoneme list. Otherwise it will be changed in the middle
		// of the clause. This way however it will not be restored to default, but there is a workaround for that - specify
		// the language og the clause explicitly
